...
class bam:
	...
	def generate_primer_bcf(self,threads=4,flank=30):
		...
		cmd = "cut -f4 %(primer_bed_file)s | parallel -j %(threads)s \"bcftools mpileup  -f %(ref_file)s %(prefix)s.{}.bam %(mpileup_options)s -R %(primer_bed_file)s | bcftools call -T %(primer_bed_file)s %(vtype)s -mg %(min_dp)s | bcftools norm -f %(ref_file)s  | bcftools +setGT -Ob -o %(prefix)s.{}.bcf -- -t q -i 'FMT/DP<%(min_dp)s' -n ." % self.params
		run_cmd(cmd)

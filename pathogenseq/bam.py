...
class bam:
	...
	def generate_primer_bcf(self,threads=4,flank=30):
		...
		for l in open(self.params["primer_bed_file"]):
			...
			run_cmd(cmd)
			cmd = "cat %(primer_bed_file)s | parallel --col-sep '\t' -j %(threads)s \"bcftools mpileup  -f %(ref_file)s %(bam_file)s %(mpileup_options)s -r {1}:{2}-{3} | bcftools call %(vtype)s -m | bcftools +setGT -Ob -o %(prefix)s.{4}.bcf -- -t a -n .\"" % self.params

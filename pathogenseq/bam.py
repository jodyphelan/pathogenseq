	...
	def generate_primer_bcf(self,threads=4,flank=30):
		...
		for l in open(self.params["primer_bed_file"]):
			...
			if read_num==0:
				...
				cmd = "samtools index %(prefix)s.{}.bam && bcftools mpileup  -f %(ref_file)s %(prefix)s.{}.bam %(mpileup_options)s -r %(tmp)s | bcftools call -t %(tmp)s %(vtype)s -mg %(min_dp)s | bcftools norm -f %(ref_file)s  | bcftools +setGT -Ob -o %(prefix)s.%(tmp)s.bcf -- -t q -i 'FMT/DP<%(min_dp)s' -n .\"" % self.params
			run_cmd(cmd)
